...
class Airkit_Envato_API {
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'gowatch' ), '1.0.0' );
	}
	...
	public function request( $url, $args = array() ) {
		$defaults = array(
			'headers' => array(
				...
				'User-Agent' => 'WordPress - Airkit Library',
			),
			'timeout' => 20,
		);
		...
		if ( empty( $this->token ) ) {
			return new WP_Error( 'api_token_error', __( 'An API token is required.', 'gowatch' ) );
		}
		...
		if ( 200 !== $response_code ) {
			return new WP_Error( $response_code, __( 'An unknown API error occurred.', 'gowatch' ) );
		}
		$return = json_decode( wp_remote_retrieve_body( $response ), true );
		if ( null === $return ) {
			return new WP_Error( 'api_error', __( 'An unknown API error occurred.', 'gowatch' ) );
		}
		return $return;
	}
	...
	public function deferred_download( $id ) {
		...
		return add_query_arg( $args, esc_url( admin_url( 'admin.php?page=yani' ) ) );
	}
