 ...
 *
 * @class Airkit_Updater
 * @version 5.0.0
 * @since 5.0.0
 */
final class Airkit_Updater {

	/**
	 * The arguments that are used in the Airkit_Product_Registration class.
	 *
	 * @access private
	 * @since 1.0.0
	 * @var array
	...
	/**
	 * An instance of the Airkit_Product_Registration class.
	 *
	 * @access private
	 * @since 1.0.0
	 * @var object Airkit_Product_Registration.
	 */
	 ...
	 *
	 * @access public
	 * @param object $registration An instance of the Airkit_Product_Registration class.
	 */
	public function __construct(  ) {

		if ( null === $this->registration ) {

			$saved_token = airkit_option_value('update', 'envato_token');
			$this->registration = new Airkit_Envato_API($saved_token);
		}
	...
	public function deferred_download( $id ) {
		...
		return add_query_arg( $args, esc_url( admin_url( 'admin.php?page=pandella' ) ) );
	}
	...
	public function download( $id, $args = array() ) {
		...
		$response = $this->registration->request( $url, $args );
	...
	public function update_themes( $transient ) {
		...
		// Get the installed version of Theme.
		$current_theme = wp_get_theme();
		$current_theme_name = $current_theme->get( 'Name' );
		$current_theme_version = $current_theme->get( 'Version' );

		// Process Theme updates.
		if ( isset( $transient->checked ) && 'gowatch' === strtolower( $current_theme_name ) ) {

			// Get the themes from the Envato API.
			$themes = $this->registration->themes();
			...
			foreach ( $themes as $theme ) {
				if ( isset( $theme['name'] ) && $current_theme_name === strtolower( $theme['name'] ) ) {
					$latest_th = $theme;
					break;
				}
			}

			if ( version_compare( $current_theme_version, $latest_th['version'], '<' ) ) {
				$transient->response[ $latest_th['name'] ] = array(
					'theme'       => $latest_th['name'],
					'new_version' => $latest_th['version'],
					'url'         => 'https://touchsize.com/downloads/'.$latest_th['name'],
					'package'     => $this->deferred_download( $latest_th['id'] ),
				);
			}
		}
	...
	public function update_plugins( $transient ) {
		...
		$premiums = $this->registration->plugins();
		...
		foreach ( $plugins as $plugin_file => $plugin ) {
			...
			if ( isset( $args['bundled'] ) && ! empty( $args['bundled'] ) ) {
				foreach ( $args['bundled'] as $bundled_plugin ) {
					if ( $plugin['Name'] === $bundled_plugin && isset( $args['bundled-versions'][ $bundled_plugin ] ) && version_compare( $plugin['Version'], $args['bundled-versions'][ $bundled_plugin ], '<' ) ) {
						$_plugin = array(
	...
	/**
	 * Disables requests to the wp.org repository for Theme.
	 *
	 * @since 5.0.0
	 *
	...
	public function plugins_api( $response, $action, $args ) {
		...
		if ( 'plugin_information' === $action && isset( $args->slug ) ) {
			$installed = $this->registration->plugins();
