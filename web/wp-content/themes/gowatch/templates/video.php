...
if ( 'style2' == airkit_single_option( 'single_style' ) ) {
	get_template_part( 'templates/video2' );
	return;
}
	...
	<div class="container">
		...
		<article <?php airkit_element_attributes( $article_atts, array('element' => 'article', 'is-single' => true), $post->ID ) ?>>
			...
			<?php
				...
				if( !post_password_required() && gowatch_video_subscribers_only() !== true ) {
					...
					echo '<figure '. airkit_element_attributes( $figure_attributes, array(), $post->ID, false ) .'>';
						...
						echo '<div class="video-figure-content">';
							...
							echo airkit_single_video_player( $post->ID );
					...
					echo '<div class="logged-in-video">';
					echo '<h4>' . esc_html__('You must be logged in to see video first.', 'gowatch') . '</h4>';
					wp_login_form();
					echo '</div>';
				}
			?>
			...
			<aside class="post-meta">
				<div class="post-meta-author">
					<?php
						if ( 'y' == airkit_option_value('single', 'video_meta') && 'n' != airkit_single_option('author') ) {
			...
			<aside class="post-container">
				...
				<div class="post-content <?php echo $content_size_class; ?>">
					<?php
						...
						if ( gowatch_video_subscribers_only() !== true ) {
							the_content();
						}
